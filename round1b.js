function getRandomInt(max) {
  ...
  async function getDataFromFirestore() {
    ...
    if (userData.round == 1) {
      ...
      const querySnapshot = await db.collection("data1b").get();
      const dataArray = querySnapshot.docs.map((doc) => doc.data());
      const questionAnswerId = getRandomInt(dataArray.length);
  
      user.doc(localStorage.getItem('userId')).set({
          r1q2 : dataArray[questionAnswerId]["question"],
          r1a2 : dataArray[questionAnswerId]["answer"]
        }, {merge : true})
      ...
      document.getElementById("submit").addEventListener("click", (e) => {
        ...
        if (
          ...
          window.location.replace("round1c.html");
        } else {
      ...
      qst.innerHTML = userData.r1q2;
  
      document.getElementById("round-1").addEventListener("keyup", (e) => {
        if (
          e.target.value.toLowerCase() ==
          userData.r1a2.toLowerCase()
        ) {
          document.querySelector("#submit").style.display = "block";
        } else {
          document.querySelector("#submit").style.display = "none";
