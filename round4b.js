function getRandomInt(max) {
  ...
  async function getDataFromFirestore() {
    ...
    if (userData.round == 4) {
      ...
      const querySnapshot = await db.collection("data 4b").get();
      const dataArray = querySnapshot.docs.map((doc) => doc.data());
      const questionAnswerId = getRandomInt(dataArray.length);
  
      user.doc(localStorage.getItem('userId')).set({
        round : 4,
        r4q2 : dataArray[questionAnswerId]["question"],
        r4a2 : dataArray[questionAnswerId]["answer"]
      }, {merge : true})
      ...
      qst.innerHTML = userData.r4q2;
  
      document.getElementById("round-4").addEventListener("keyup", (e) => {
        if (
          e.target.value.toLowerCase() ==
          userData.r4a2.toLowerCase()
        ) {
          document.querySelector("#submit").style.display = "block";
        } else {
          document.querySelector("#submit").style.display = "none";
